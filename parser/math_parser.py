...
math_parser = Lark(
    ...
    function: max | maxabs | min | sum | product | map | reduce
    array_functions: map
    array_like: arrays | array_functions
    max: "max(" array_like ("," range)? ")"
    maxabs: "maxabs(" array_like ("," range)? ")"
...
def print_error_message(expression):
    ...
    """.format(
        error_msg=expression.children[0]
    )
...
#include "filtration_algorithms.h"
    ...
    for filtration in parse_tree.children:
        ...
        if filtration.children[-1].data == "cases":
            for case in filtration.children[-1].children:
                if case.children[0].type == "ELSE":
                    if (
                        hasattr(case.children[1], "data")
                        and case.children[1].data == "error"
                    ):
                        result += print_error_message(case.children[1])
                    else:
                        result += "return {code};".format(
                            code=expression_to_string(case.children[1])
                        )
                else:
                    code = ""
                    if (
                    ...
                    """.format(
                        dim=case.children[0], code=code
                    )
        else:
            result = "return {code};".format(
                code=expression_to_string(filtration.children[-1])
            )
        ...
        face_filtration_code = (
            ...
            if not uses_the_face_filtration
            else ""
        )
...
{algorithms}
    ...
    """.format(
        algorithms=algorithm_selection_code, algorithm_list=algorithm_list
    )
    return ret
...
def simple_array_operation(expression, dive_deeper):
    frm = (
        ...
        else dive_deeper(range_get_from(expression.children[1]))
    )
    to = (
        "dimension"
        if len(expression.children) == 1
        else dive_deeper(range_get_to(expression.children[1]))
    )
...
def map_(expression, dive_deeper, depth, bound_variables):
    ...
    new_bound_variables.update(
        {
            str(lambda_exp.children[0]): get_array(expression.children[0], dive_deeper)
            + "["
            + counter_name
            + "]"
        }
    ...
    return """
        ...
        counter=counter_name, frm=frm, to=to, value=value, depth=depth
    )
...
def reduce_(expression, dive_deeper, depth, bound_variables, operation="="):
    ...
    new_bound_variables.update(
        {
            ...
            str(lambda_exp.children[1]): get_array(expression.children[0], dive_deeper)
            + "["
            + counter_name
            + "]",
        }
    ...
    return """
        ...
        initial_value=dive_deeper(expression.children[-1], depth, bound_variables),
...
def expression_to_string(expression, depth=0, bound_variables={}):
    ...
    def continue_with_children(exp, d=depth, bV=bound_variables.copy()):
        if len(expression.children) > 1:
            return reduce(
                lambda carry, exp: carry + dive_deeper(exp), expression.children, ""
            )
    ...
    if expression.data in ["max", "maxabs", "min", "sum", "product"]:
        ...
        modified_tree = Tree(
            "reduce",
            expression.children[0:-1]
            + [
                Tree(
                    "lambda",
                    [Token("VAR", "_c_a_r_r_y")] + expression.children[-1].children,
                ),
                Token("SIGNED_NUMBER", base_value),
            ],
        )
    ...
    if expression.data == "factor":
        if (
            hasattr(expression.children[0], "type")
            and expression.children[0].type == "EXP"
        ):
            return "exp({expr})".format(expr=dive_deeper(expression.children[1]))
        if (
            hasattr(expression.children[0], "data")
            and expression.children[0].data == "base"
            and len(expression.children) > 1
        ):
            return "pow({base}, {exp})".format(
                base=dive_deeper(expression.children[0]),
                exp=dive_deeper(expression.children[2]),
            )

    if expression.data == "array_element":
        if (
            hasattr(expression.children[0], "data")
            and expression.children[0].data == "global_arrays"
        ):
            if expression.children[0].children[0].type == "VERTEX_OUT_DEGREES":
                return "graph.outdegrees[(int)({idx})]".format(
                    idx=dive_deeper(expression.children[1])
                )
            if expression.children[0].children[0].type == "VERTEX_IN_DEGREES":
                return "graph.indegrees[(int)({idx})]".format(
                    idx=dive_deeper(expression.children[1])
                )
