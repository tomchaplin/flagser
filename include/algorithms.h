...
struct maxabs_filtration : public filtration_algorithm_t {
    virtual inline value_t compute_filtration(unsigned short dimension, const directed_flag_complex_cell_t& cell,
                                    const filtered_directed_graph_t& graph,
                                    const value_t* boundary_filtration) const {
        return (maxabs(boundary_filtration, 0, dimension));
    }
};
...
if (algorithm == "maxabs") return new maxabs_filtration();
...
if (algorithm == "vertex_degree") return new vertex_degree_filtration();
    ...
    std::vector<std::string> custom_filtration_computer{ "dimension", "zero", "max", "maxabs", "max3", "max_plus_one", "product", "sum", "pmean", "pmoment", "remove_edges", "vertex_degree" };
